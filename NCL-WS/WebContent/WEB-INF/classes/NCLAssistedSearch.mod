 ...
 *********************************************/
  ...
  */
   ...
   dexpr float TotalRecommendationLimitViolation = ( PackageRecommendationSlack + ThingsToDoSlack +
   ...
   dexpr float TotalItineraryUniquenessLimitViolation = sum(p in Input_Parameters) ItinerarySlack * p.ItineraryUniquenessPriority;

   
   // Contribution attributed to accommodation violation
   // 	WC1 * WB1 * ( FP1 - FC1 )   
   dexpr float AccommodationPreferenceContribution = sum(p in Input_Parameters) p.CustomerAccommodationPriority * 
    			 						   			 p.BusinessAccommodationPriority * FP1;

   // Contribution attributed to Things To Do violation
   //   WC2 * WB2 * ( FP2 - FC2 )   
   dexpr float ThingsToDoPreferenceContribution =    sum(p in Input_Parameters) p.CustomerThingsToDoPriority *
    		     						   			 p.BusinessThingsToDoPriority * FP2;
   
   // Contribution attributed to Price violation
   //   WC3 * WB3 * FC3
   dexpr float PriceConstraintViolation =  sum(p in Input_Parameters) p.CustomerPricePriority *
    		     						   p.BusinessPricePriority * FC3;
    
   // Contribution attributed to Cruise Duration violation
   //   WC4 * WB4 * ( FC41 + FC42 )    
   dexpr float DurationConstraintViolation = 	sum(p in Input_Parameters) p.CustomerTimePriority * 
    		     						   		p.BusinessTimePriority * ( FC41 + FC42 );
   
   // Contribution attributed to Destination violation
   //   WC5 * WB5 * FC5   
   dexpr float DestinationConstraintViolation = sum(p in Input_Parameters) p.CustomerDestinationPriority *
    		     						   		p.BusinessDestinationPriority * FC5;
   
   // Contribution attributed to Departing Port violation
   //   WB6 * FC6   
   dexpr float DepartingPortConstraintViolation = sum(p in Input_Parameters) p.BusinessDepartingPriority * FC6;
  ...
  {   
    //FC1
    //forall(i in Input_CruisePackage)
    //	CruisePackageFC1[i] == sum(m in Input_MetaRange:m.Meta==i.Meta) 1;
    
    //FC5
    //forall(i in Input_CruisePackage)
    //  	CruisePackageFC5[i] == sum(d in Input_DestinationRange: d.Destination==i.Destination) 1;
