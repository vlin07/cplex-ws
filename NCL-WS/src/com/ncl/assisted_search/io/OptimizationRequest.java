	 ...
	 if(properties == null)
	 {
		...
		try {
			InputStream inputStream = OptimizationRequest.class.getClassLoader().getResourceAsStream("app.properties");
			 System.out.println("attempting create ne properties() object");
			 System.out.println("attempting to load the inputStream into the properties Object");
		...
		oplInputDataFile=properties.getProperty("oplInputDataFile");
		 ...
		 System.out.println("attempting createDataSource " + properties.getProperty("OplDataSource"));
		
		//dataSource = oplF.createOplDataSource(properties.getProperty("OplDataSource"));		

		 System.out.println("attempting createModelSource " + properties.getProperty("OplModelSource"));

		//		modelSource = oplF.createOplModelSource(properties.getProperty("OplModelSource"));

		 System.out.println("finish modelSource ");

		//def = oplF.createOplModelDefinition(modelSource, settings);

		System.out.println("finish def ");
     ...
     try {
        ...
        Connection conn = DriverManager.getConnection(
		 ...
		 if(userInput._sailDateRangeList.size()==0){
        	 ...
        	 query3 = query3+" AND DC_SAIL_START-TO_DATE("+ft.format(currentTime.getTime())+", 'YYYYMMDD')<=90)";
		 }
		 query3=query3+") ";
...
*/
	...
	{	
		...
		if (cplex.solve()){
	        ...
	        opl.postProcess();
	        if(DEBUG) System.out.println("objDef="+objDef);
			if(objDef==0 || objDef==2){
